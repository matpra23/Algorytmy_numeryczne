...
def macierze_lag(): 
    ...
    for i in range(0,21,3):
        przedzial_x=x[i:i+3+1]
        przedzial_y=f[i:i+3+1]
        n=len(przedzial_x) 
        a=interp_lag(przedzial_x,przedzial_y,n) 
        xx=np.linspace(przedzial_x[0],przedzial_x[n-1],100) 
        yy=np.zeros(100)
        nn=len(xx)
        for i in range(nn):
            yy[i]=a_lag(a,przedzial_x,xx[i]) 
...
print("Całka metodą prostokątów z aproksymacji : ", calkaAproksymacjaKwadratowa)